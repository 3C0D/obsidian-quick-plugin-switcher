...
import { getSelectedContent, hasKeyStartingWith, isEnabled } from './utils.ts';
import {
	CPModal,
	getManifest,
	getPluginsList,
...
import { Filters, Groups, SortBy, TargetPlatform } from './types/variables.ts';
import {
	getPluginsInGroup,
	editGroupName,
	groupMenu,
	addRemoveGroupMenuItems,
	addToGroupSubMenu,
...
export const mostSwitchedResetButton = (
	...
	if (
		...
		Object.keys(installed).some((id) => installed[id].switched !== 0)
	) {
...
export const Check4UpdatesButton = (modal: QPSModal, el: HTMLSpanElement): void => {
	...
	const toUpdate = Object.values(modal.plugin.settings.installed).filter(
		(item) => item['toUpdate']
	);
...
export const checkbox = (
	...
	contentEl.createDiv({ text: text, cls: 'qps-comm-invert' }, (el) => {
		el.createEl(
			...
			(checkbox) => {
				((checkbox.checked = isQPS
					? settings.byAuthor
					: settings.invertFiltersComm),
					(checkbox.onchange = () => {
						isQPS
							? (settings.byAuthor = checkbox.checked)
							: (settings.invertFiltersComm = checkbox.checked);
						plugin.saveSettings();
						reOpenModal(modal);
					}));
			}
...
async function searchUpdates(modal: QPSModal): Promise<void> {
	...
	for (const item of Object.values(installed)) {
		...
		const filePath = modal.app.vault.adapter.getFullPath(item.dir);
	...
	new ButtonComponent(el)
		...
		.buttonEl.addEventListener('click', (evt: MouseEvent) => {
			...
			if (plugin.lengthEnabled === 1 && settings.wasEnabled.length === 0) {
						...
						.onClick(async () => {
							...
							if (plugin.lengthEnabled > 1) {
								...
								for (const i of settings.wasEnabled) {
									...
									if (toUpdate) {
										await conditionalEnable(modal, toUpdate);
				...
				menu.addSeparator();
				menu.addItem((item) =>
					item.setTitle('Toggle enabled-plugins by group').setDisabled(true)
				);
...
export const openGitHubRepo = async (
...
export const searchDivButtons = (modal: QPSModal, contentEl: HTMLElement): void => {
...
export const searchCommDivButton = (modal: CPModal, contentEl: HTMLElement): void => {
	// toggle plugin options
	contentEl.createEl(
		'span',
		{
	...
	if (modal instanceof QPSModal) {
		if (groups[groupNumber]) {
			if (!groups[groupNumber].hidden && !inGroup.length) {
				new Notice('empty group', 3000);
				return;
			}
			groups[groupNumber].hidden = !groups[groupNumber].hidden;
		}
		inGroup.forEach((id) => {
			if (groups[groupNumber].hidden) installed[id].groupInfo.hidden = true;
			else {
				let prevent = false;
...
export function handleDblClick(
	evt: MouseEvent | TouchEvent,
	modal: QPSModal | CPModal,
	element?: HTMLElement
): void {
	...
	if (pluginItemBlock) {
		const matchingItem = findMatchingItem(
			modal,
			pluginItemBlock.parentElement as HTMLElement
		);
		if (!matchingItem) return;
		handleInputDblClick(
			modal as QPSModal,
			pluginItemBlock,
			matchingItem as PluginInstalled
		);
	}
...
export async function handleContextMenu(
	...
	if (modal instanceof QPSModal) {
		targetBlock = elementFromPoint?.closest('.qps-item-line') as HTMLElement;
	} else {
		targetBlock = elementFromPoint?.closest('.qps-comm-block') as HTMLElement;
	}
...
export function contextMenuCPM(
	...
	if (this.app.isMobile) {
		...
		menu.addItem((item) => {
			item.setTitle('Remove All groups')
				.setDisabled(matchingItem.groupCommInfo.groupIndices.length === 0)
				.onClick(async () => {
	...
	if (isInstalled(matchingItem.id)) {
		...
		if (Platform.isDesktop) {
			...
			if (filePath) {
				...
				isDevPlugin = fs.existsSync(packageJsonPath);
			}
		}

		menu.addItem((item) => {
			let disabled = false;
			disabled = matchingItem.id === 'quick-plugin-switcher' || isDevPlugin;
		...
		if (Platform.isDesktop) {
			const text = matchingItem.commandified
				? 'Remove command from plugin'
				: 'Add command to plugin';
			const disabled =
				(Platform.isDesktop && matchingItem.target === TargetPlatform.Mobile) ||
				(Platform.isMobile && matchingItem.target === TargetPlatform.Desktop);
			menu.addItem((item) => {
				item.setTitle(text)
		...
		if (!this.app.isMobile) {
			menu.addItem((item) => {
				...
				submenu.addItem((subitem) => {
					subitem
						.setTitle('All groups')
						.setDisabled(matchingItem.groupInfo.groupIndices.length === 0)
						.onClick(async () => {
...
export async function updatePlugin(
	...
	if (!(id in commPlugins)) {
		...
		new Notice(
			`can't update, version ${manifest.version} in repo has not been released!`
		);
	} else if (lastVersion <= version) {
		new Notice(`Already last version ${lastVersion}`, 2500);
	} else {
		try {
			await modal.app.plugins.installPlugin(
				commPlugins[id!].repo,
				lastVersion,
				manifest
			);
...
export async function addCommandToPlugin(
	...
	const disabled =
		(Platform.isDesktop && pluginItem.target === TargetPlatform.Mobile) ||
		(Platform.isMobile && pluginItem.target === TargetPlatform.Desktop);
	plugin.addCommand({
